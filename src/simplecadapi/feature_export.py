...
def _sanitize_freecad_identifier(name: str) -> str:
...
def _matrix_expr(transform: Any) -> str:
    rows = []
    for row in transform:
        rows.extend(float(value) for value in row)
    values = ", ".join(repr(value) for value in rows)
    return f"App.Matrix({values})"


def _placement_expr_from_matrix(transform: Any) -> str:
    return f"App.Placement({_matrix_expr(transform)})"


def generate_freecad_assembly_script(assembly_or_result: Any) -> str:
    """Generate a FreeCAD script from a solved AssemblyResult or Assembly."""
    is_assembly = hasattr(assembly_or_result, "_parts") and hasattr(assembly_or_result, "_order")
    result = assembly_or_result.solve() if hasattr(assembly_or_result, "solve") else assembly_or_result

    if not hasattr(result, "part_names") or not hasattr(result, "get_solid"):
        raise ValueError("assembly_or_result must be an Assembly or AssemblyResult")

    part_names = list(getattr(assembly_or_result, "_order", result.part_names()))
    lines = [
        "# FreeCAD Python assembly script generated by SimpleCADAPI",
        "# This script imports solved assembly parts as FreeCAD objects",
        "",
    ...
    if is_assembly:
        assembly = assembly_or_result
        group_names: Dict[str, str] = {}

        for index, part_name in enumerate(part_names):
            node = assembly._parts[part_name]
            brep = _extract_shape_brep_string(node.solid)
            obj_name = f"AssemblyPart_{index:03d}_{_sanitize_freecad_identifier(part_name)}"
            part_container = f"{obj_name}_Part"
            shape_name = f"{obj_name}_Shape"
            shape_object = f"{obj_name}_ShapeObject"
            placement = (
                node.local_transform
                if node.parent is not None
                else assembly._world_transform(part_name)
            )

            group_names[part_name] = part_container
            lines.extend([
                f"# Assembly Part: {part_name}",
                f"{part_container} = doc.addObject('App::Part', '{part_container}')",
                f"{part_container}.Label = {part_name!r}",
                f"{part_container}.Placement = {_placement_expr_from_matrix(placement)}",
            ])
            ...
            if brep:
                ...
                lines.append(f"# Warning: BREP export unavailable for assembly part {part_name}")

            if node.parent is not None:
                parent_container = group_names[node.parent]
                lines.append(f"{parent_container}.addObject({part_container})")
            lines.append("")
    else:
        for index, part_name in enumerate(part_names):
            ...
            lines.append(f"# Assembly Part: {part_name}")
            if brep:
                lines.extend([
                    f"{shape_name} = Part.Shape()",
                    f"{shape_name}.importBrepFromString({brep!r})",
                    f"{obj_name} = doc.addObject('Part::Feature', '{part_name}')",
    ...
    lines.extend([
        ...
        "",
        f"print('Assembly imported successfully with {len(part_names)} parts')",
        "",
    ])
    return "\n".join(lines)


def export_assembly_result_to_freecad_script(assembly_or_result: Any, filepath: str) -> str:
    """Export a solved assembly result to a flat FreeCAD Python script."""
    script_content = generate_freecad_assembly_script(assembly_or_result)
